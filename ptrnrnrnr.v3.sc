...
~additiveBus = Bus.audio(s, 2);
~additive_sawtoothBus = Bus.audio(s, 2);


SynthDef(\verb_my, {
	Out.ar(0, FreeVerb.ar(In.ar(~additiveBus, 2), mix: 1.0, room: 0.9, mul: 0.9));
}).add;
...
SynthDef(\additive, {
    ...
    sig = {
        Mix.fill(n, { arg index;
			...
			SinOsc.ar(
				(freq_curr + LFNoise1.kr({Rand(5.0, 15.5)}).range(0.0, freq_curr*0.005*env_harm)),
				0,
				(amp_base + SinOsc.kr(
	...
	Out.ar(~additiveBus, (sig*env/n) ! 2);
}).add;
...
SynthDef(\additive_sawtooth, {
    ...
    sig = {
        Mix.fill(n, { arg index;
            ...
            freq_curr = frq_base*(index+1);
            freq_curr.postln;
			frq_mod_frq = {Rand(5.0, 15.5)}.value;
			amp_mod_frq = {Rand(0.0, 0.5)}.value;
			SinOsc.ar(
				(freq_curr + SinOsc.kr(
	...
	Out.ar(out, (sig*env/n) ! 2);
	Out.ar(~additive_sawtoothBus, (sig*env/n) ! 2);
}).add;
...
~additiveSaw = Synth(\additive_sawtooth, [\freq, 1400, \amp, 0.4]);
...
(
    ...
    ~verbOne = Synth(\verb_my);
    ~binStretch_bus_additive = Synth(\binStretch_bus, [\in_bus, ~additiveBus]);
    ~binStretch_bus_additive_sawtooth = Synth(\binStretch_bus, [\in_bus, ~additive_sawtoothBus]);
    {
	    ~additivePtrn = Pbind(
		    \instrument, \additive,
		    \freq, Pwhite(100, 1500, inf),
	    ...
	    ~additivePtrn5.play;
	    ~verbOne.free;
	    ~binStretch_bus_additive_sawtooth.free;
    }.fork(~clock1);
)
